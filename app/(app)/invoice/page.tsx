...
export default function InvoiceListPage() {
  ...
  const [creatorFilter, setCreatorFilter] = useState("Semua");
  ...
  const uniqueCreators = Array.from(
    new Set(invoices.map((i) => i.created_by_name?.trim()).filter(Boolean))
  ) as string[];
  ...
  const filteredInvoices = invoices.filter((inv) => {
    ...
    const matchesCreator =
      creatorFilter === "Semua" ||
      (inv.created_by_name || "Admin") === creatorFilter;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-3">
        ...
        {/* Filter Creator Dropdown */}
        {uniqueCreators.length > 0 && (
          <div className="shrink-0">
            <select
              value={creatorFilter}
              onChange={(e) => setCreatorFilter(e.target.value)}
              className="w-full sm:w-auto bg-white border border-slate-200 px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-700 focus:outline-none focus:border-blue-500 transition shadow-sm"
            >
              <option value="Semua">Semua Pembuat</option>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
                    ...
                    <span className="inline-flex items-center gap-1 text-[11px] font-medium bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md border border-slate-200">
                      <User className="w-3 h-3 text-slate-400" /> {inv.created_by_name || "Admin"}
                    </span>
                  </div>
                </div>
              </div>
